			...
			while ( $slides->have_posts() ) : $slides->the_post();
				...
				$slider .= '<li>';
					...
					$slider .= '<div id="slide-' . get_the_ID() . '" class="slide">';
						...
						$slider .= '<div class="slide-title"><a href="' . get_post_meta( $post->ID, "_slide_link_url", true ) . '" title="' . the_title_attribute ( array( 'echo' => 0 ) ) . '" >' . get_the_title();
                                                $slider .= '<br/><span>' . get_post_meta( $post->ID, "_captionx", true ).'</span> </a></div>';
					$slider .= '</div><!-- #slide-x -->';
...
function responsive_slider_metabox_1() {
	...
	$captionx = get_post_meta( $post->ID, '_captionx', true ); ?>
	...
	<p>caption: <input type="text" style="width: 90%;" name="captionx" value="<?php echo esc_attr( $captionx ); ?>" /></p>
	<span class="description"><?php echo _e( 'description', 'responsive-slider' ); ?></span>
...
function responsive_slider_save_meta( $post_id, $post ) {
	...
	if ( isset( $_POST['slide_link_url'] ) ) {
        ...
        if ( isset( $_POST['captionx'] ) ) {
		update_post_meta( $post_id, '_captionx', strip_tags( $_POST['captionx'] ) );
	}
}
